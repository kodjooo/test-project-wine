...
## Этап 2 — Crawler
- `CategoryCrawler` создаёт отдельную страницу Playwright и обходит очередь URL с учётом посещённых страниц.
...
- Изображения берутся из `src`/`srcset` в блоке `.product__content-img`, сохраняется список и основное `hero_image_url`.
...
- Данные подготавливаются в `SheetRecord` по фиксированной схеме колонок (см. docs/requirements.md).
- Upsert осуществляется по `PRODUCT_ID`: поиск строки, обновление или добавление через batch-update/append.
- Формула `IMAGE_CELL` записывается как `=IMAGE(IMAGE_DIRECT_URL)`, используются прямые ссылки FreeImage.host, а рядом сохраняются viewer/thumbnail URL и SHA-256.
